"""Broader PCH regression matrix: real compilation and distinct GPU outputs.
...
CASES = ("inline_simple", "inline_complex", "load_files", "plain_cpp", "flag_miss")
...
code = subprocess.call([os.environ["PCH_TEST_COMPILER"], *sys.argv[1:]])
if "-c" in sys.argv:
    with open(os.environ["PCH_TEST_COMMANDS"], "a") as f:
...
def trial(case: str, variant: int, enabled: bool):
    import hashlib
    import os
    import re
    import shlex
...
(root / "result.json").write_text(json.dumps(dict(seconds=seconds, outputs=outputs)))
'''
            env = os.environ | {
            ...
            correctness_checks = 6
            times = [wall]
        observed = [json.loads(line) for line in commands.read_text().splitlines()]
        hit_keys = re.findall(r"\[kernelbot-pch\] hit (\w+)", log)
        misses = "[kernelbot-pch] miss" in log
        expected_hit = enabled and case in {"inline_simple", "inline_complex", "load_files"}
        ...
        nvcc_commands = []
        if ninja.exists():
            rendered = subprocess.check_output(["ninja", "-C", str(root), "-t", "commands"], text=True)
            nvcc_commands = [shlex.split(line) for line in rendered.splitlines() if "/nvcc " in line]
        return {
            "case": case, "variant": variant, "enabled": enabled, "extension_name": name,
            ...
            "nvcc_translation_units": len(nvcc_commands),
            "remote_seconds": time.perf_counter() - started,
        }


@app.local_entrypoint()
def main(output: str = "/tmp/kernelbot-pch-matrix.json", cases: str = ""):
    ...
    assert len(hit_keys) == (1 if set(selected) & {"inline_simple", "inline_complex", "load_files"} else 0)
    for case in selected:
